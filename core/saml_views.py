...
def authenticate(saml_authentication=None):
    ...
    if saml_authentication.is_authenticated():
        user_data = {'first_name': None, 'last_name': None, 'username': None, 'email': None }
        ...
        user = api.user_exists(user_data)
        if user:
            if not api.is_user_active(user_data['username']):
                raise SuspiciousOperation
        else:
...
def attrs(request):
    ...
    return render(request, 'core/attrs.html', {
        'paint_logout': paint_logout,
        'attributes': attributes
    })
