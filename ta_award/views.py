...
from .forms import AccessLevelForm, UserForm, CustomUserEditForm
...
def index(request):
    return render(request, 'ta_award/index.html', {
...
def get_accepted_apps(request):
    ...
    return render(request, 'ta_award/get_accepted_apps.html', {
        'apps': apps,
...
class UsersView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'ta_award/get_users.html', {
            'users': users,
...
class EditUserView(View):
    ...
    custom_user_edit_form_class = CustomUserEditForm
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not api.has_custom_user_created(user):
            user.customuser = CustomUser.objects.using('default').create(user_id=user.id)

        return render(request, 'ta_award/edit_user.html', {
            'user': user,
            'user_form': self.user_form_class(data=None, instance=user),
            'custom_user_edit_form': self.custom_user_edit_form_class(data=None, instance=user.customuser)
        })
    ...
    def post(self, request, *args, **kwargs):
        ...
        custom_user_edit_form = self.custom_user_edit_form_class(request.POST, instance=user.customuser)

        if user_form.is_valid() and custom_user_edit_form.is_valid():
            if user_form.save():

                # Update access levels of this user
                old_accesslevels = user.customuser.accesslevels.all()
                user.customuser.accesslevels.remove( *old_accesslevels )
                new_accesslevels = list(custom_user_edit_form.cleaned_data.get('accesslevels'))
                user.customuser.accesslevels.add( *new_accesslevels )
...
class AccessLevelsView(View):
    ...
    @method_decorator(require_GET)
    def get(self, request, *args, **kwargs):
        return render(request, 'ta_award/get_accesslevels.html', {
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect('ta_award:get_accesslevels')
