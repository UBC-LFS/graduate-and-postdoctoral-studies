...
from .models import AccessLevel, CustomUser
...
def get_user_accesslevels(user):
    ...
    accesslevels = []
    for al in user.customuser.accesslevels.all():
...
def create_customuser(user):
    ''' Create an user's member '''
    guest_accesslevel = AccessLevel.objects.using('default').filter(name='Guest').first()

    custom_user_form = CustomUserForm({ 'accesslevels': [guest_accesslevel.id] })
    if custom_user_form.is_valid():
        custom_user = CustomUser.objects.using('default').create(user_id=user.id)
